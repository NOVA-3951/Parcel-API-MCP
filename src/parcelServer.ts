...
export function createParcelMcpServer(apiKey: string): McpServer {
  ...
  server.tool(
    ...
    {
      tracking_number: z.string().describe("Tracking number for the delivery"),
      carrier_code: z
        .string()
        .describe(
    ...
    async ({
      ...
      try {
        ...
        return {
          content: [
            { type: "text" as const, text: `Error adding delivery: ${errorMessage}` },
          ],
          isError: true,
        };
      }
      ...
      try {
        ...
        if (data.deliveries && Array.isArray(data.deliveries)) {
          const formattedDeliveries = data.deliveries.map((delivery: any) => ({
            ...delivery,
            status_description:
              DELIVERY_STATUS_CODES[delivery.status_code] || "Unknown status",
          }));
  ...
  server.tool(
    ...
    async () => {
      try {
        ...
        return {
          content: [
            { type: "text" as const, text: JSON.stringify(carriers, null, 2) },
          ],
        };
      } catch (error) {
        const errorMessage =
  ...
  server.tool(
    ...
    async () => {
      const statusInfo = Object.entries(DELIVERY_STATUS_CODES).map(
        ([code, description]) => ({ code: parseInt(code), description })
      );
      return {
        content: [
          { type: "text" as const, text: JSON.stringify(statusInfo, null, 2) },
        ],
      };
    }
  );
